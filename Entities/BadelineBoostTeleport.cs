...
namespace Celeste.Mod.DJMapHelper.Entities
{
    public class BadelineBoostTeleport:Entity
    {
        ...
        public BadelineBoostTeleport(Vector2[] nodes, string normalRoom, string normalColorGrade, string keyRoom, string keyColorGrade, string goldenRoom, string goldenColorGrade, bool keyFirst)
          : base(nodes[0])
        {
            Depth = -1000000;
            this.nodes = nodes;
            ...
            Collider = (Collider) new Circle(16f, 0.0f, 0.0f);
            Add((Component) new PlayerCollider(new Action<Player>(OnPlayer), (Collider) null, (Collider) null));
            Add((Component) (sprite = GFX.SpriteBank.Create("badelineBoost")));
            Add((Component) (this.stretch = new Image(GFX.Game["objects/badelineboost/stretch"])));
            stretch.Visible = false;
            stretch.CenterOrigin();
            Add((Component) (light = new VertexLight(Color.White, 0.7f, 12, 20)));
            Add((Component) (bloom = new BloomPoint(0.5f, 12f)));
            Add((Component) (wiggler = Wiggler.Create(0.4f, 3f, (Action<float>) (f => sprite.Scale = Vector2.One * (float) (1.0 + (double) wiggler.Value * 0.400000005960464)), false, false)));
            Add((Component) (relocateSfx = new SoundSource()));
        }
        ...
        {
            Add((Component) new Coroutine(BoostRoutine(player), true));
        }
        ...
        private IEnumerator BoostRoutine(Player player)
        {
            ...
            if (finalBoost)
            {
                ...
                Add((Component) new Coroutine(level.ZoomTo(center, 1.5f, 0.18f), true));
            ...
            if (finalBoost)
            {
                Scene.Add((Entity) new CS_BoostTeleport(player, this, normalRoom, normalColorGrade, keyRoom, keyColorGrade, goldenRoom, goldenColorGrade, keyFirst));
                player.Active = false;
            ...
            Add((Component) Alarm.Create(Alarm.AlarmMode.Oneshot, (Action) (() =>
            {
                if (player.Dashes < player.Inventory.Dashes)
                    ++player.Dashes;
                Scene.Remove((Entity) badeline);
                (Scene as Level).Displacement.AddBurst(badeline.Position, 0.25f, 8f, 32f, 0.5f, (Ease.Easer) null, (Ease.Easer) null);
            }), 0.15f, true));
            ...
            if (!finalBoost)
            {
                ...
                tween.OnUpdate = (Action<Tween>) (t =>
                {
                    ...
                    if ((double) t.Eased >= 0.899999976158142 || !Scene.OnInterval(0.03f))
                        return;
                    TrailManager.Add((Entity) this, Player.TwoDashesHairColor, 0.5f, false, false);
                    level.ParticlesFG.Emit(BadelineBoost.P_Move, 1, Center, Vector2.One * 4f);
                });
                tween.OnComplete = (Action<Tween>) (t =>
                {
                    if ((double) X >= (double) level.Bounds.Right)
                    {
                        RemoveSelf();
                    }
                    else
                ...
                Add((Component) tween);
                relocateSfx.Play("event:/char/badeline/booster_relocate", (string) null, 0.0f);
                Input.Rumble(RumbleStrength.Strong, RumbleLength.Medium);
                level.DirectionalShake(-Vector2.UnitY, 0.3f);
                level.Displacement.AddBurst(Center, 0.4f, 8f, 32f, 0.5f, (Ease.Easer) null, (Ease.Easer) null);
                tween = (Tween) null;
            }
            else
            {
                ...
                level.Displacement.AddBurst(Center, 0.6f, 8f, 64f, 0.5f, (Ease.Easer) null, (Ease.Easer) null);
                level.ResetZoom();
        ...
        {
            wiggler.Start();
            (Scene as Level).Displacement.AddBurst(Position, 0.3f, 4f, 16f, 0.25f, (Ease.Easer) null, (Ease.Easer) null);
        ...
        private void Finish()
        {
            SceneAs<Level>().Displacement.AddBurst(Center, 0.5f, 24f, 96f, 0.4f, (Ease.Easer) null, (Ease.Easer) null);
